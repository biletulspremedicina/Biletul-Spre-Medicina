...
export function ChemistryBlockRenderer({ block }: { block: ChemistryLessonBlock }) {
  if (block.block_type === 'heading') {
    return <h2 id={`bloc-${block.id}`} className="scroll-mt-24 text-2xl font-bold text-[#123f34] sm:text-3xl">{block.content}</h2>;
  }
  if (block.block_type === 'subheading') {
    return <h3 id={`bloc-${block.id}`} className="scroll-mt-24 text-xl font-bold text-[#205e4e] sm:text-2xl">{block.content}</h3>;
  }
  if (block.block_type === 'paragraph') {
    return <p className="whitespace-pre-wrap text-[16px] leading-8 text-stone-700">{block.content}</p>;
  }
  if (block.block_type === 'list') {
    return (
      <ul className="space-y-2 pl-1 text-[16px] leading-7 text-stone-700">
        {block.items.filter(Boolean).map((item, index) => (
          <li key={index} className="flex gap-3"><span className="mt-2.5 h-1.5 w-1.5 shrink-0 rounded-full bg-brand-600" />{item}</li>
        ))}
      </ul>
    );
  }
  if (block.block_type === 'note') {
    ...
    return (
      <aside className={`flex gap-3 rounded-2xl border-l-4 p-5 ${styles}`}>
        <Icon className="mt-0.5 shrink-0" size={22} />
        <p className="whitespace-pre-wrap leading-7">{block.content}</p>
      </aside>
    );
  }
  if (block.block_type === 'formula') {
    return (
      <div className="overflow-x-auto rounded-2xl border border-[#d6e8e1] bg-[#f7fcf9] px-6 py-5 text-center text-xl text-[#123f34] sm:text-2xl">
  ...
  if (block.block_type === 'image' && block.image_url) {
    return (
      <figure className="rounded-2xl border border-stone-200 bg-white p-3 shadow-sm">
        ...
        {block.caption && <figcaption className="px-3 pb-1 pt-3 text-center text-sm text-stone-500">{block.caption}</figcaption>}
      </figure>
    );
  }
  return null;
