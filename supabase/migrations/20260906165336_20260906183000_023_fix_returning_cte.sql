/*
# Fix: use WITH ... AS (INSERT ...) to avoid RETURNING INTO ambiguity
*/
...
RETURNS TABLE (
  out_id uuid, out_user_id uuid, out_sim_id uuid, out_answers jsonb,
  out_score integer, out_max_score integer, out_started_at timestamptz,
  out_submitted_at timestamptz, out_expired boolean, out_expires_at timestamptz
)
...
BEGIN
  ...
  IF v_sim.requires_subscription = true THEN
    ...
    SELECT i.id, i.user_id, i.simulation_id, i.answers, i.score, i.max_score,
           i.started_at, i.submitted_at, i.expired, i.expires_at
      INTO r_id, r_user_id, r_sim_id, r_answers, r_score, r_max_score,
           r_started_at, r_submitted_at, r_expired, r_expires_at
    FROM ins i;
